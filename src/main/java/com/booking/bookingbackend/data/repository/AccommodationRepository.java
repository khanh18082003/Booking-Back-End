...
public interface AccommodationRepository extends BaseRepository<Accommodation, UUID> {
    ...
    @Query(value = """
            WITH available_accommodations AS (
                SELECT
                    accommodation_id,
                ...
                HAVING COUNT(DISTINCT date) = :nights
                 ...
                 valid_accommodations AS (
                     ...
                     GROUP BY a.id, a.name, a.capacity, aa.min_available_rooms, aa.total_price, rg.rooms, al.amenities
                     ORDER BY :capacity -a.capacity
                 )
            SELECT *
