...
public class GoogleMapService {
    ...
    public JsonNode getLocation(String q, String limit) {
        try {
            ...
            String url = String.format(
                    ...
                    input, "lodging%7Cpolitical", GOOGLE_MAPS_API_KEY, limit);
            ...
            log.error("Unexpected error while calling Google Maps API: {}", ex.getMessage());
            return null;
        }
    }
    ...
    public double[] getLatLng(String location) {
        try {
            ...
            String url = String.format(
                    "https://maps.googleapis.com/maps/api/geocode/json?address=%s&key=%s", input, GOOGLE_MAPS_API_KEY);
            ...
            if (responseBody != null) {
                ...
                if (status.equals("OK")) {
                    JsonNode locationNode =
                            rootNode.path("results").get(0).path("geometry").path("location");
