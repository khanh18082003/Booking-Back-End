...
public class BookingServiceImpl implements BookingService {
    ...
    @Transactional
    @Override
    public BookingResponse book(BookingRequest request) throws MessagingException, UnsupportedEncodingException {
        // Validate request
        ...
        final var aAccommodations = request.accommodations().stream()
                .map(accommodation -> {
                    var availabilities = accommodationService.checkAvailabilityForBooking(
                            accommodation.id(), request.checkIn(), request.checkOut(), accommodation.quantity());
        ...
        booking.setProperties(propertiesRepository
                .findById(request.propertiesID())
                .orElseThrow(
                        () -> new AppException(ErrorCode.MESSAGE_INVALID_ENTITY_ID, Properties.class.getSimpleName())));
        if (request.userId() != null) {
            booking.setUser(userRepository
                    .findById(request.userId())
                    .orElseThrow(
                            () -> new AppException(ErrorCode.MESSAGE_INVALID_ENTITY_ID, User.class.getSimpleName())));
        }
    ...
    public List<BookingResponse> BookingHistory(UUID userId) {
        ...
        return bookingList.stream().map(mapper::toDtoResponse).collect(Collectors.toList());
    }

    @PreAuthorize("hasRole('USER')")
    @Override
    public PaginationResponse<UserBookingsHistoryDTO> getUserBookingsHistory(int pageNo, int pageSize) {
        CustomUserDetails userDetails = SecurityUtils.getCurrentUser();
        User user = userDetails.user();

        Pageable pageable = PageRequest.of(pageNo - 1, pageSize);
        Page<UserBookingsHistoryDTO> page = repository.findUserBookingsHistory(user.getId(), pageable);
    ...
    public void cancelBooking(UUID bookingId) {
        ...
        List<BookingDetail> bookingDetailList = bookingDetailRepository.findAllByBookingId(booking.getId());
        ...
        bookingDetailList.forEach(bookingDetail -> {
            List<Available> availables = availableRepository.findAllByAccommodationIdBetweenCheckInAndCheckOut(
                    bookingDetail.getId().getAccommodationId(), checkIn, checkOut.minusDays(1));
    ...
    @Override
    public PaginationResponse<BookingDetailResponse> getAllBookingsByPropertiesId(String id, int pageNo, int pageSize) {
        Pageable pageable = PageRequest.of(pageNo - 1, pageSize, Sort.by(Direction.DESC, "created_at"));

        if (id.equals("all")) {
            CustomUserDetails userDetails = SecurityUtils.getCurrentUser();
            var bookingPage = repository.findAllOfHost(userDetails.user().getId(), pageable);
            ObjectMapper objectMapper = new ObjectMapper();
            List<BookingDetailResponse> result = bookingPage.stream()
                    .map(booking -> {
                        try {
            ...
            Properties properties = propertiesRepository
                    ...
                    .orElseThrow(() ->
                            new AppException(ErrorCode.MESSAGE_INVALID_ENTITY_ID, Properties.class.getSimpleName()));
            var bookingPage = repository.findAllByPropertiesId(properties.getId(), pageable);
            ObjectMapper objectMapper = new ObjectMapper();
            List<BookingDetailResponse> result = bookingPage.stream()
                    .map(booking -> {
                        try {
