...
public interface BookingService extends BaseEntityService<UUID, Booking, BookingRepository, BookingResponse> {
    ...
    BookingResponse book(BookingRequest bookingRequest) throws MessagingException, UnsupportedEncodingException;

    BookingResponse changeStatus(UUID id, BookingStatus status);

    List<BookingResponse> BookingHistory(UUID userId);

    PaginationResponse<UserBookingsHistoryDTO> getUserBookingsHistory(int pageNo, int pageSize);

    void cancelBooking(UUID bookingId);

    PaginationResponse<BookingDetailResponse> getAllBookingsByPropertiesId(String id, int pageNo, int pageSize);
