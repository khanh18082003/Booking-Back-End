...
public class PaymentServiceImpl implements PaymentService {
    ...
    public PaymentResponse save(PaymentRequest request) {
        ...
        payment.setBooking(bookingRepository
                .findById(request.bookingId())
                .orElseThrow(
                        () -> new AppException(ErrorCode.MESSAGE_INVALID_ENTITY_ID, Booking.class.getSimpleName())));
    ...
    public Boolean checkPaymentOnlineStatus(UUID id, int expectedAmount, String expectedTransactionId)
        ...
        for (int i = 0; i < transactions.length(); i++) {
            ...
            String transactionContent =
                    transaction.getString("transaction_content").trim();
