...
public class AccommodationServiceImpl implements AccommodationService {
    ...
    public AccommodationResponse save(final AccommodationCreationRequest request) {
        ...
        if (rooms != null) {
            Set<AccommodationHasRoom> accommodationHasRooms = rooms.stream()
                    .map(room -> {
                        // Lấy roomType
                        var roomType = roomTypeRepository
                                .findById(room.roomTypeId())
                                .orElseThrow(() -> new AppException(
                        ...
                        Set<RoomHasBed> roomHasBeds = room.bedTypes().stream()
                                .map(bt -> {
                                    var bedType = bedTypeRepository
                                            .findById(bt.id())
                                            .orElseThrow(() -> new AppException(
                                                    ErrorCode.MESSAGE_INVALID_ENTITY_ID,
                                                    getEntityClass().getSimpleName()));
        ...
        response.setImages(imageList.stream().map(Image::getUrl).collect(toList()));

        // Set amenities cho response
        response.setAmenities(savedEntity.getAmenities().stream()
        ...
        response.setRooms(savedEntity.getAccommodationHasRooms().stream()
    ...
    public void saveAll(List<AccommodationBookingResponse> aDays) {
        ...
        for (var accommodation : aDays) {
            for (var availableAccommodation : accommodation.getAvailableAccommodations()) {
                var availableEntity = availableRepository
                        .findById(availableAccommodation.id())
                        .orElseThrow(() ->
                                new AppException(ErrorCode.MESSAGE_INVALID_ENTITY_ID, Available.class.getSimpleName()));
                availableEntity.setTotalReserved(availableEntity.getTotalReserved() + accommodation.getQuantity());
    ...
    public List<AvailableAccommodationDTO> checkAvailabilityForBooking(
            final UUID id, final LocalDate checkInDate, final LocalDate checkOutDate, Integer quantity) {
        ...
        final var aDays = availableRepository.findAndLockAvailableAccommodation(
                id, checkInDate, checkOutDate.minusDays(1), quantity);
    ...
    public AccommodationResponse update(final UUID id, final AccommodationUpdateRequest request) {
        Accommodation accommodation = repository
                .findById(id)
                .orElseThrow(() -> new AppException(
                        ErrorCode.MESSAGE_INVALID_ENTITY_ID, getEntityClass().getSimpleName()));
        ...
        response.setRooms(updatedAccommodation.getAccommodationHasRooms().stream()
                .map(room -> RoomResponse.builder()
                        .id(room.getId())
                        .roomName(room.getRoomName())
                        .roomType(room.getRoomType().getName())
                        ...
                        .build())
                .collect(Collectors.toSet()));
        response.setAmenities(updatedAccommodation.getAmenities().stream()
                .map(a -> AmenitiesResponse.builder()
                        .id(a.getId())
                        .name(a.getName())
                        .build())
        ...
        response.setImages(imageRepository.findAllByReferenceId(id.toString()).stream()
                .map(Image::getUrl)
                .collect(toList()));
        return response;
    }
    ...
    @Override
    public List<AccommodationSearchDTO> findAccommodationByPropertyId(final AccommodationsSearchRequest request) {
        var id = request.id();
        var rooms = request.rooms();
        ...
        List<AccommodationSearchDTO> accommodationSearchDTOList = tuples.stream()
                .map(row -> {
                    try {
                        ...
                        List<AmenityDTO> amenityList = objectMapper.readValue(amenitiesJson, new TypeReference<>() {});
                        ...
                        List<String> imageUrls =
                                objectMapper.readValue(row.get("image_urls", String.class), new TypeReference<>() {});
                        // Trả về DTO
    ...
    public List<AccommodationHostDTO> getAccommodationsByPropertyId(UUID id) {
        ...
        return accommodations.stream()
                .map(accommodation -> {
                    List<String> images =
                            imageRepository
                                    .findAllByReferenceId(accommodation.getId().toString())
                                    .stream()
                                    .map(Image::getUrl)
                                    .collect(toList());
                    List<RoomDTO> rooms = accommodation.getAccommodationHasRooms().stream()
                            .map(room -> new RoomDTO(
                                    ...
                                    room.getRoomHasBedList().stream()
                                            .map(bed ->
                                                    new BedDTO(bed.getBedType().getName(), bed.getQuantity()))
                                            .toList()))
                            .toList();
    ...
    @PreAuthorize("hasRole('HOST')")
    @Override
    public List<AvailableResponse> updatePriceAvailableByDate(AvailableUpdatePriceRequest request) {
        List<Available> availableList =
                availableRepository.findAllByAccommodationIdAndDate(request.id(), request.dates());
        ...
        return availableRepository.saveAll(availableList).stream()
